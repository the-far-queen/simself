# SimSelf v6.0 — Python Source (extracted from Grok chat)

**Date:** 2026-09-15
**Source:** `Desktop/Grok/grok1.txt` — Bobby's prompt, lines 1-898 of the file.
**Topic:** SimSelf Unified v6.0 — Constitutional Identity Substrate (single file)
**Authors (per file header):** Robert Wolfson, Claude, DeepSeek — 2026 merge pass.
**License:** MIT (per file header)

---

## Bobby's prompt (verbatim)

> new simself pls run in sandbox, apply figuratively to urself whats good novel strong snd whats not?

---

## Python source (verbatim, with markdown code block)
...
    """
    def **init**(self, axes: Optional[List[Tuple[str, int]]] = None):
...
    """
    def **init**(self, dim: int = DIM, use_torch: bool = True, seed: int = 42):
        self.dim = dim
...
    def **call**(self, delta: np.ndarray) -> np.ndarray:
        if self.use_torch:
            with torch.no_grad():
...
    def **init**(self, dim: int = TEXT_EMBED_DIM, decay_rate: float = 0.01, threshold: float = 0.3,
                 max_entries: int = 200):
        self.dim = dim
...
    """
    def **init**(self, constitution: Optional[Constitution] = None,
...
    def **init**(self, agent: Optional[Callable] = None,
...
class FieldCore:
    def **init**(self, world_model: Optional[Callable] = None,
...
## Notes from the source file's docstring (verbatim, header)

- **8 design changes** made during the merge (per `EVAL_NOTES.md` referenced in source):
  1. FIXED: psi_0 is genuinely immutable after init (v4/v4.1 bug — untrained net overwrote it).
  2. FIXED: token-level hashing embedding (instead of whole-string SHA256) so "resonance by similarity" works.
  3. MERGED: full 20-axis / 7-sheaf layout restored (v4 dropped 10 axes without stated reason).
  4. MERGED: v4's egg-toroid axial curvature gradient reprojected onto 7-sheaf structure.
  5. HONESTY FIX: "learnable Resolution Operator" honestly labeled (with no training loop, it's a fixed random nonlinear damping).
  6. HARDENED: keyword safety checks use word-boundary regex (not naive substring) — flagged as advisory only.
  7. IMPROVED: Harness._is_coherent uses cosine similarity (not raw word-count threshold).
  8. KEPT AS-IS: ResonantMemory decay/threshold retrieval, AtlasExam five tests, CLI, Harness control-loop shape.

Runs with: **numpy only** (required, ubiquitous). torch is optional and auto-detected.
